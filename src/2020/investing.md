...
それぞれのお得度をあえて「雑」に点数で表現するならば:

| 枠                                  | お得度 |
| ----------------------------------- | ------ |
| 1. iDeco （上限あり）               | 140 点 |
| 2. つみたて NISA （年 40 万円まで） | 120 点 |
| 3. 特定口座でインデックスファンド   | 100 点 |
...
- 現金とリスク資産の間には相関関係はないですので、現金比率をいくつにしようと、ア
...
現金 <input type="range" min="1" max="400" value="400" class="slider"> リスク資
産
...
現金 <input type="range" min="1" max="400" value="200" class="slider"> リスク資
産
...
1. A さんは毎月貯金していた 16 万円のうち、50%である 8 万円を資産運用に回すこと
   にしました。
2. A さんは iDeCo を はじめました。月々の拠出額は最大の 23,000 円です。
...
現金 <input type="range" min="1" max="100" value="100" class="slider"> リスク資
産
...
現金 <input type="range" min="1" max="100" value="0" class="slider"> リスク資産

B さんの今の手取りは 20 万円です。B さんは生活費として 15 万円使用します。つまり
毎月 5 万円貯金できます。

1. B さんは iDeCo を はじめました。月々の拠出額は最大の 23,000 円です。
2. B さんは つみたて NISA をはじめました。最大のつみたて額は設定できないので、毎
   月 27,000 円、つみたて NISA に設定しました。
...
- 「暴落はまだ始まったばかりです。これから株価はもっと下げます。全部、現金にしま
  しょう！」
- 「インデックス投資なんて対して儲かりませんよ！短期間で大儲けするなら、FX です
  ！！
- 「仮想通貨で 1 億円儲かりました！」
...
- インデックスファンドは株式市場をまるごと買う行為です。つまり、インデックスファ
  ンドをひたすら買うことは、資産運用の成績は、市場平均とほぼ同じになります。詳し
  くは後述。
...
- つまり、すべての情報を織り込んだ・市場全体の総意によって決定されている株価はい
  ...
  要はありませんし予想する必要もありません。株の買い時・売りどきはありません。あ
  えて言うなら、365 日 24 時間、いつだって（リスク許容度の範囲内で）「買いどき」
  と仮定しましょう。
...
- われわれが株を買うときその裏で株を売っている相手は巨大な専門機関のトップクラス
  であり、われわれが株を買うときその裏で株を売っている相手もプロです。
...
# どうして時価総額加重平均にもどづくインデックスファンドが良いのでしょうか？なにか理論的な裏付けがあるのでしょうか？
...
## 期待リターン(mean), 中央値(median; u)、中央値 ± 標準偏差 (x1, x2) のチャート (x 軸: 年数, y 軸: 成績 (対数))
...
これらのチャートは、株価は対数正規分布だというモデルにもどついて描きました。
...
# アセットアロケーションに債権を入れたほうがよいといわれたのですが？

「普通の人」はリスク資産は株 100%でいいと思います。

「株と債権」などのようにリスク資産を 2 種類以上持ち、それを組み合わせてアセット
アロケーションをつくるのは、普通の人にとっては難しすぎると思いますし、特別に大き
いメリットもなさそうです。

それよりは、アセットアロケーションをシンプルに保ち、簡単にリスクとリターンを可視
化・把握できるようにしておいたほうがメリットが大きいでしょう。
...
```
δ^2 = 0.2^2 + 0.1^ + 2 * 0 * 0.2 * 0.1 = 0.05
δ = sqrt(0.05) = 0.224 (= 22.4%)
```

つまり為替リスクの貢献度は 2.4% ほどです。為替リスクが 10%だからといって、そのま
ま 10% 為替リスクが増えるわけではありません。そのため（内部で）ドル建ての商品に
...
1. A さんは相続（あるいは贈与等）で 1000 万円を **現金** を手にいれました。
2. A さんは相続（あるいは贈与等）で 1000 万円分の **株** を入手しました。

A さんのリスク許容度はいぜんとして、現金:リスク資産 = 50:50 です。それぞれのケー
スにおいて、 A さんはどのようにアセットアロケーションにするのがよいでしょうか？

この 2 通りのケースに対して、もしあなたが:

### 「1000 万円を **現金** を手にいれました」 の場合:
...
- 利益確定のために売却する必要はありません。「利益確定」という概念は捨てましょう
  。資産運用は一生続くものだと考えましょう。
- 売却する必要があるるのは、アセットアロケーションのバランスを保つときだけだと考
  えましょう。
