...
。この記事ではリターンは 中央値 (μ (50%)) のことを指していることが多いですが、質
問者はリターンを「宝くじのリターンは 3 億円です！」と同じ意味で、すわなち「もっ
ともうまくいったときのリターン」、 (μ ではなく 例えば μ + 5σ ) という意味で使用
している可能性が高いです。

そのような意味でのリターン (μ + 5σ) の大きさを追求するのであれば資産運用などしな
いで「宝くじにフルインベストメント」しましょう。

もちろん宝くじの正しい意味でのリターン μ は実際にはマイナスです。
...
という考えからでしょう。そのような特定の商品・インデックスの特定の期間の「過去リ
ターン」を理由に商品を選択するのは、典型的な「余計なこと」です。

なお「半分ずつもつ」というのは実際にはそこまで投資効率を落とすというわけではあり
ません。それぞれの商品が 99 点 vs 98 点だとしたら、半分ずつもつのは は 97 点くら
いといってよいでしょう。

なお、「半分ずつもった」からといって「結果のリターン」がちょうど両者の成績の中間
になるとは限りません。実際には「どちらにも負ける」ことが起こりえます。結果が「単
純に足して 2 で割った成績」、いわゆる算術平均にならないのは、 50% + 50% のバラン
スを保つために定期的にリバランス等を行うからです。もしリバランスを行わないのであ
れば、それはもはやポートフォリオとは呼べるものではなく、もはやこの議論の対象外で
す。:)
...
これはみなさん中学生のときに学習したおなじみの上に凸な二次関数です。

TODO: x 軸: $ L $, y 軸: $ M_t $ のチャートを書く。

つまり、レバレッジ倍率 L を大きくしていっても、リターンの中央値・対数期待値は途
中から伸び悩み、それどころか、しまいには、逆にリターンが下がっていきます。
